  ...
  formatToString(results) {
    ...
    for (let i = 0; i < results.tests.length; i++) {
      ...
      if (test.diagnostics.length > 0 && test.testNumber < results.totalTests) {
        suitechanged = true
        out += `\n  ${underline(test.diagnostics[0])}\n\n`
        out += formatDiagnostics(test.diagnostics.slice(1))
      } else if (test.diagnostics.length > 0) {
        out += formatDiagnostics(test.diagnostics)
      }
    }
