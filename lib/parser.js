...
// the indent separates a runner's own structure from a test's output: brittle names a test in a
// comment at column 0 and indents whatever the test itself prints
const diagnostic = /^(\s*)#(.*)$/
    ...
    switch (this.state) {
      ...
      case STORE_TEST_METADATA: {
        ...
        if (testLineMatch) {
          ...
          if (diagMatch) {
            const indented = diagMatch[1].length > 0
            const diagnosticLine = (diagMatch[2] || '').trim()
            if (diagnosticLine !== '' && !runnerComment.test(diagnosticLine)) {
              if (this.currentTest !== null) {
                this.currentTest.diagnostics.push(diagnosticLine)
                if (!indented) this.currentTest.groups.push(diagnosticLine)
              } else if (indented) {
                results.leadingDiagnostics.push(diagnosticLine)
              } else {
                results.explanation.push(diagnosticLine)
              }
              events.push({ type: 'comment', text: diagnosticLine, indented })
            }
          }
        }
        break
...
function isPassing(r) {
  ...
  return r.expectedTests < 0 || r.totalTests >= r.expectedTests
}
