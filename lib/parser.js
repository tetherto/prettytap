    ...
    switch (this.state) {
      ...
      case STORE_TEST_METADATA: {
        ...
        if (testLineMatch) {
          ...
          if (this.topLevelTests === results.expectedTests) {
            this.foundAllTests = true
          }

          events.push({ type: 'test', test: this.currentTest })
        } else if (yamlStart.test(line) && this.currentTest !== null) {
          this.state = STORE_YAML
        } else {
      ...
      case STORE_YAML: {
        ...
        } else if (bailOutLine.test(line) || testPlanDeclaration.test(line)) {
          // ponytail: both anchor at column 0, so real (indented) YAML values never match
          if (this.currentTest !== null && this.currentTest.yamlBytes.length > 0) {
            events.push({ type: 'yaml', test: this.currentTest })
          }
          this.state = STORE_TEST_METADATA
