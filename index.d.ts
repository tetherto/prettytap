...
export class SpecFormatter implements Formatter {
  ...
  streamTest(test: Test): string
  streamYaml(test: Test): string
  streamBail(reason: string): string
  formatTest(test: Test, prefix?: string): string
  formatDetail(test: Test): string
  format(results: Results): void
  summary(): void
}
